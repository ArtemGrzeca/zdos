...
unsigned int placement_address = (unsigned int)&kernel_end;
...
void *alloc(unsigned int size, unsigned char page_align, heap_t *heap)
{
    ...
    if (iterator == -1) // If we didn't find a suitable hole
    {
        ...
        // Find the  end most header. (Not end most in size, but in location).
        iterator = 0;
        // Vars to hold the index of, and value of, the end most header found so far.
