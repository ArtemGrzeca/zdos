...
{
    unsigned int mem_end_page = ram_size;
	...
	idt_set_gate(14, (unsigned long)page_fault, 0x08, 0x8E);
...
void page_fault(regs_t* regs)
{
	...
	char* buffer[128];
	xsprintf(buffer, "Page fault at 0x%08x (P=%d RW=%d U=%d R=%d)", faulting_address, present, rw, us, reserved);
	PANIC(buffer);
}
